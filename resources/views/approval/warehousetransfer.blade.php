...
@section('content')
    <div class="container">
        ...
        <div class="container bg-white mt-5 shadow rounded p-3">
            ...
            <h4 class="card-title mt-5">Warehouse / Distribution Center Stock Request Approve</h4>
            <!-- Search -->
            <x-datatable-search />
    ...
    @endforeach
                                    ...
                                    @if ($first->status == 1 && $first->requested_by_warehouse_id == $userWarehouseId)
                                        <form method="POST" action="{{ route('warehouse.transfer.receive.bulk') }}"
                                            class="d-inline">
                                            @csrf
                                            @foreach ($group as $t)
                                                <input type="hidden" name="transfer_ids[]" value="{{ $t->id }}">
                            ...
                            @foreach ($group as $t)
                                <tr>
                                    ...
                                    <td>{{ $t->challan?->items->where('product_id', $t->product_id)->first()?->quantity ?? $t->quantity }}
                                    </td>
                                    <td>
                                        @if ($t->status == 0)
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-sm btn-success">Dispatch</button>
                                            ...
                                            <form method="POST" action="{{ route('warehouse.transfer.reject', $t->id) }}"
                                                class="d-inline">
                                                @csrf
