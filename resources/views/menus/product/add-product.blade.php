                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        @if($mode === 'view')
                                        <input type="hidden" id="view_gst" value="{{ $product->tax->gst ?? 0 }}">
                                        <input type="text"
                                            class="form-control"
...
<script>
    function calculateFinalPrice() {
        ...
        // ✅ Edit / Add Mode
        if ($('[name="tax_id"]').length) {
            gstPercent = parseFloat($('[name="tax_id"] option:selected').data('gst')) || 0;
        }
        // ✅ View Mode
        else if ($('#view_gst').length) {
            gstPercent = parseFloat($('#view_gst').val()) || 0;
        }
        ...
        // ✅ Calculation
        const gstAmount = +(sellingPrice * gstPercent / 100).toFixed(2);
        ...
        if (sellingPrice < basePrice) {
            $('[name="retailer_price"]').addClass('is-invalid');
        } else {
            $('[name="retailer_price"]').removeClass('is-invalid');
        }

        // ❌ Final > MRP
        if (mrp && finalPrice > mrp) {
            $('#final_price').addClass('is-invalid');
        } else {
            $('#final_price').removeClass('is-invalid');
        }
    }
</script>
...
<script>
    $(document).ready(function() {
        ...
        function setGST() {
            let gst = $('select[name="tax_id"] option:selected').data('gst') || 0;
            $('#gst_percentage').val(gst);
        }
